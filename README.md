...
The canonical skill source lives in `blueprint/skills-source/`; `.agents/skills`
and `.claude/skills` are the tool adapters generated from it.
...
└── blueprint/
    ...
    ├── skills-source/         (canonical source for workflow skills)
    ├── scripts/
    │   └── sync-skills.mjs    (copies skills-source to tool adapters)
    ├── context/
...
because the tools that read them look there. The workflow source lives under
`blueprint/`, so it stays out of your app code.

When editing workflow skills, change `blueprint/skills-source/<skill>/SKILL.md`
first, then regenerate the adapters:

```bash
node blueprint/scripts/sync-skills.mjs
```
...
`blueprint/skills-source` is the canonical workflow source, `.agents/skills`
exposes the workflow to Codex, and `.claude/skills` exposes it to Claude Code.
